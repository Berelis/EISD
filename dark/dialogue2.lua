...
function get_reponse(contexte,sujet,question)
	...
	bdd[contexte][sujet] ~= nil then
		...
		if #question.reponse[#question.reponse] == 0 then 
			print("Nous n'avons pas la reponse a cette question (" .. sujet .. " du " .. contexte .. ").")
		-- Sinon
		else	
			-- Si la reponse n'est pas un tableau
			if question.reponse[#question.reponse][1] == nil then
				print("La reponse a votre question portant sur le sujet '" .. sujet .. "' et concernant le pays '" .. contexte .. "' est : " .. question.reponse[#question.reponse])
			-- Si la reponse est un tableau de taille un
			elseif #question.reponse[#question.reponse] == 1 then
				print("La reponse a votre question portant sur le sujet '" .. sujet .. "' et concernant le pays '" .. contexte .. "' est : " .. question.reponse[#question.reponse][1])
			-- Si la reponse est un tableau de taille > 1
			else
				print("Les reponses a votre question portant sur le sujet '" .. sujet .. "' et concernant le pays '" .. contexte .. "' sont :")
				for j = 1, #question.reponse[#question.reponse] do
					print(question.reponse[#question.reponse][j])
				end
			end	
		end
	else
		print("Nous ne gerons pas encore cette question (" .. sujet .. " concernant le pays " .. contexte .. ").")
	end
end
...
function get_reponse_base(sujet,question)
	...
	question.reponse[#question.reponse + 1] = resultat
...
----------------------------------------------------------------------------------------------------
		...
		if #question.contexte == 0 and #question.sujet == 0 and question.base == false then
		...
		elseif (#question.contexte ~= 0 or question.base) and #question.sujet == 0 then
			...
			(question.base == false or listequestions[questionnumber-1].jointure == false)
			then
				-- Si elle en a un
		...
		elseif question.base == false and #question.contexte == 0 and #question.sujet ~= 0 then
			...
			(listequestions[questionnumber-1].base == false or question.jointure == false) then
				-- Si elle en a un
				if listequestions[questionnumber-1].contexte[1] == nil then
					print("Votre question concerne le meme pays que pour la question precedante : " .. listequestions[questionnumber-1].contexte)
...
----------------------------------------------------------------------------------------------------
		...
		if (question.base == false and #question.contexte == 0) or #question.sujet == 0 then
		...
		-- Sinon on peut chercher une reponse
			...
			if #question.contexte ~= 0 then
				...
				if question.jointure == false then
